...
bool Point::operator==(const Point& rhs) const { return (rhs.x == x && rhs.y == y && rhs.z == z); }

/**
 * Simple Operator with float
 */

Point Point::operator+(const float& rhs) const { return {x + rhs, y + rhs, z + rhs}; }
...
Point Point::operator+(const Point& rhs) const { return {x + rhs.x, y + rhs.y, z + rhs.z}; }
...
float Point::dot(const Point& rhs) const { return x * rhs.x + y * rhs.y + z * rhs.z; }
